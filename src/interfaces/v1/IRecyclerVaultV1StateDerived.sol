...
interface IRecyclerVaultV1StateDerived {
    /// @notice The max request possible for `account`.
    function maxRequest(address account) external view returns (uint256 assets);
    /// @notice Preview the `shares` burned by requesting `assets`.
    function previewRequest(uint256 assets) external view returns (uint256 shares);
    /// @notice Returns the rollover status. If true, the vault is depositable, otherwise reverts.
