...
interface IRewardsHash {
    ...
    /// @return latestClaimable lastest claimable hash for specified cycle, cycle latest hash
    ///     (possibly non-claimable) for specified cycle    
    function cycleHashes(uint256 index) external view returns (
        string memory latestClaimable,
        string memory cycle
    );
